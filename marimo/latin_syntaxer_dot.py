...
def _(mo):
    mo.md("""
    # Explore a saved analysis as a Graphviz diagram
    """)
    return
...
def _(Path, mo):
    ...
    # format -- see notes/install.md and USAGE.md's "Saving and loading
...
def _(sentences, split_analysis_by_sentence, tokengraph, verbalunits):
    ...
    # ever has to think about "the currently selected sentence's tokengraph".
    sentence_slices = []
    split_error = None
    if sentences:
        try:
...
# latin_syntaxer_review.py's own sentence_label() uses.
...
def _(mo):
    ...
    # directly so you can see rank_by_depth's effect (Graphviz's `rank=same`
    # forcing same-depth verbal expressions onto one level -- the whole
    # point of this notebook, see notes/dot_diagrams.md) by toggling it.
    orientation_dropdown = mo.ui.dropdown(
        options=["BT", "TB", "LR", "RL"],
        value="BT",
        label="*Orientation*:",
    )
    rank_checkbox = mo.ui.checkbox(value=True, label="Rank by AAT depth (`rank=same`)")
...
def _(
    ...
    # latin_syntaxer_review.py's own otherwise-identical cell uses for
    # maxdepth.
    ...
    if not selected_tokengraph:
        ...
        dot_display = mo.callout(
            mo.md(
                ...
                "-- see notes/install.md), or download the `.dot` source "
...
def _(dot_filename_stem, dot_source, mo, selected_tokengraph):
    ...
    # latin_syntaxer_review.py's own mermaid_download uses. Renderable
...
def _():
    ...
    # way `aat` is for latin_syntaxer_review.py's AAT graph: importable or
    # not, checked once here, rather than every display cell catching
