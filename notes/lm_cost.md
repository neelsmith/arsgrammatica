...
`marimo/` has 7 notebooks; 4 of them actually make LM calls (`latin_syntaxer_ctsdata.py`, `latin_syntaxer_selected_ids.py`, `latin_syntaxer_textinput.py`, `latin_syntaxer_tokenized.py` -- each configures a `dspy.LM` and calls `analyze_sources()`/`analyze_string()`/`analyze_selected_passages()`/`analyze_with_retry()`), and all 4 now show a cost. The other 3 (`latin_syntaxer_graph_metrics.py`, `latin_syntaxer_review.py`, `lewis_short_lookup.py`) never import `dspy` at all -- the first two only browse a previously-saved analysis file (`read_analyses()`), and the third only browses a Lewis & Short lexicon file, with no LM involved anywhere in any of them, so there's no `lm.history` for them to report and no cost cell was added to them. (Verified by grep, not assumption: `grep -n "import dspy\|dspy\." marimo/latin_syntaxer_graph_metrics.py marimo/latin_syntaxer_review.py marimo/lewis_short_lookup.py` returns nothing.)
...
`latin_syntaxer_ctsdata.py`, `latin_syntaxer_selected_ids.py`, and `latin_syntaxer_textinput.py`'s "Analysis" section each have a single `cost_summary = summarize_lm_cost(lm.history)` cell (with a `_ = results` reactivity line -- see above) in place of the old `last_call`/`cost` pair, and their "See cost" display cell calls `format_lm_cost(cost_summary)`.
