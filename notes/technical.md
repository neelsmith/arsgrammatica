...
- `arsgrammatica/` — the package with the actual analysis logic:
  ...
  - `token_budget.py` — `estimate_max_tokens()` picks a `max_tokens` budget for a SyntaxAnalysis call from a passage's token count, using a fit `calibrate_max_tokens.py` writes to `token_budget_calibration.json` (or an untuned fallback before that's ever been run); `analyze_with_retry()` wraps `analyze()` with that estimate and retries with a larger budget if the result still comes back truncated (see "Estimating and enforcing a `max_tokens` budget" above). `DEFAULT_FLOOR` (900, raised from an earlier 256) guards against a calibrated fit's own intercept being small or near-zero for very short passages -- `calibrate_max_tokens.py` only ever measures the 3-22 token range `GOLD_EXAMPLES` covers, so a fit whose line happens to start low still shouldn't be trusted to extrapolate all the way down to a 1-2 token passage; the floor keeps every real call starting at least as generous as the untuned fallback's own baseline, regardless of what any particular calibration happened to fit.
...
- `docs/build_api_docs.py` — regenerates `docs/arsgrammatica-api-docs.html`, a single self-contained HTML page documenting every name in `arsgrammatica.__all__`, built with `pdoc` straight from the package's own docstrings and type hints. Run `python3 docs/build_api_docs.py` after changing a public docstring or signature to refresh it; requires `pdoc` (`pip install pdoc --break-system-packages`).
