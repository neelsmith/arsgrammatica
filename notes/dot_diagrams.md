...
`latin_syntaxer_review.py` is the notebook that displays this diagram interactively: browse for a previously-saved analysis file (`read_analyses()`'s own format), pick a sentence from a menu (`split_analysis_by_sentence()`), and generate/display a Mermaid, Graphviz, or displaCy-style diagram for it, via a `diagram_tool` radio (`"mermaid"`/`"graphviz"`/`"displacy"` -- `"graphviz"` offered only when the `graphviz` package actually imports, same as `latin_syntaxer_textinput.py`'s and `latin_syntaxer_ctsdata.py`'s own; `"displacy"` (see `notes/displacy_viz.md`) always offered, since it has no external dependency at all). No LM access needed -- it only reads an already-saved analysis. Whichever tool is selected, the notebook degrades visibly through both Graphviz failure modes rather than crashing the cell: the `graphviz` package missing entirely (`pip install -e ".[dev]"` covers it -- see `notes/install.md`) versus the package present but the `dot` executable not on PATH (`graphviz.ExecutableNotFound`, only raised once you actually try to render) -- either way you still get a "Download diagram source" button to render elsewhere. displaCy needs neither check: `tokengraph_to_displacy_svg()` is already a complete, ready-to-display SVG the moment it's computed, so its own download button offers the `.svg` directly rather than an unrendered source format.

`latin_syntaxer_textinput.py` (a passage typed or pasted in directly) and `latin_syntaxer_ctsdata.py` (a passage selected from a CTS-served corpus) each offer the identical three-way `diagram_tool` radio, wired the same way -- same conditional `"graphviz"` offer, same always-on `"displacy"`, same `diagram_display`/`diagram_download` branching, and the same `.svg` direct-download for displaCy. `latin_syntaxer_selected_ids.py` and `latin_syntaxer_tokenized.py` do not (yet) offer displaCy.
