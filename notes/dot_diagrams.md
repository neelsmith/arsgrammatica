...
`latin_syntaxer_review.py` is the notebook that displays this diagram interactively: browse for a previously-saved analysis file (`read_analyses()`'s own format), pick a sentence from a menu (`split_analysis_by_sentence()`), and generate/display a Mermaid, Graphviz, or displaCy-style diagram for it, via a `diagram_tool` radio (`"mermaid"`/`"graphviz"`/`"displacy"` -- `"graphviz"` offered only when the `graphviz` package actually imports, same as `latin_syntaxer_ctsdata.py`'s own; `"displacy"` (see `notes/displacy_viz.md`) always offered, since it has no external dependency at all). No LM access needed -- it only reads an already-saved analysis. Whichever tool is selected, the notebook degrades visibly through both Graphviz failure modes rather than crashing the cell: the `graphviz` package missing entirely (`pip install -e ".[dev]"` covers it -- see `notes/install.md`) versus the package present but the `dot` executable not on PATH (`graphviz.ExecutableNotFound`, only raised once you actually try to render) -- either way you still get a "Download diagram source" button to render elsewhere. displaCy needs neither check: `tokengraph_to_displacy_svg()` is already a complete, ready-to-display SVG the moment it's computed, so its own download button offers the `.svg` directly rather than an unrendered source format.

`latin_syntaxer_ctsdata.py` (a passage selected from a CTS-served corpus) offers the identical three-way `diagram_tool` radio, wired the same way as `latin_syntaxer_review.py` -- same conditional `"graphviz"` offer, same always-on `"displacy"`, same `diagram_display`/`diagram_download` branching, and the same `.svg` direct-download for displaCy. `latin_syntaxer_textinput.py` (a passage typed or pasted in directly) offers the exact same three tools, wired the exact same way, but calls the Graphviz-rendered one `"dot"` rather than `"graphviz"` -- a display-label difference only, matching this codebase's own naming for the same thing elsewhere (`tokengraph_to_dot()`, dot.py, `analysis_to_dot.py`); the underlying rendering, availability check, and both failure-mode fallbacks are identical to `latin_syntaxer_review.py`'s/`latin_syntaxer_ctsdata.py`'s own `"graphviz"` option. `latin_syntaxer_selected_ids.py` and `latin_syntaxer_tokenized.py` do not (yet) offer displaCy.

Its depth slider (labelled "Maximum depth of subordination to display", `verbal_units.max_subordination_depth()`-bounded) feeds every viz on the page from one shared value: `tokengraph_to_html()`'s/`tokengraph_to_depth_html()`'s own `depth`, and `tokengraph_to_mermaid()`'s/`tokengraph_to_dot()`'s/`tokengraph_to_displacy_svg()`'s `aat_depth` -- the three notions agreeing hop-for-hop on well-formed input (see `verbal_units.compute_aat_depths()`'s own docstring), so one slider reads as one consistent cutoff everywhere despite the different parameter names. This is a DIFFERENT depth notion from `tokengraph_to_dot()`'s own graph-distance `depth` parameter (see "Depth filtering" above) -- the notebook's slider is never wired to that one.

Every other marimo notebook that draws a Mermaid or Graphviz diagram (`latin_syntaxer_textinput.py`, `latin_syntaxer_ctsdata.py`, `latin_syntaxer_selected_ids.py`, `latin_syntaxer_tokenized.py`) wires its own depth-of-subordination slider into `aat_depth` the same way -- one shared `depth` value derived from the slider, consumed by the text display, the indented-HTML view, and every diagram cell alike (and, for `latin_syntaxer_textinput.py`/`latin_syntaxer_ctsdata.py`, `tokengraph_to_displacy_svg()`'s own `aat_depth` too). In each of these (unlike `latin_syntaxer_review.py`, which already had its `finaltokens`-equivalent, `selected_tokengraph`, computed in its own upstream cell), `finaltokens` is deliberately pulled out into its own standalone cell rather than computed inside the Mermaid-diagram cell as it originally was -- the depth slider's own cap (`max_subordination_depth(finaltokens)`) already depends on `finaltokens`, so feeding the slider's value back into the SAME cell that produces `finaltokens` would create a reactive dependency cycle (`finaltokens -> maxdepth -> depth -> diagram -> finaltokens`). Extracting `finaltokens` into its own cell keeps the dependency one-way: `finaltokens` cell -> `maxdepth` cell -> `depth` cell -> the Mermaid/DOT/indented-HTML cells, never the other way around.
