...
### Ranking by depth of subordination

By default, the diagram's layout also respects each verbal expression's own *depth of subordination* (see "Depth of subordination" above) -- so independent clauses, the clauses one level below them, and so on, tend to land level with each other instead of wherever Mermaid's layout engine would otherwise scatter them. This is on by default; pass `rank_by_depth=False` for the old, unranked layout:

```python
diagram, warnings = tokengraph_to_mermaid(result.tokengraph, rank_by_depth=False)
```

Under the hood, every verbal-unit anchor node (any token with its own `verbalunitid` -- an "implied sum" or "continued discourse" implied token counts, since either anchors its own verbal expression same as a real verb would; an "implied subject" implied token does not, since it stands in for a missing noun/pronoun rather than a missing verb, and so is never ranked) at the same depth gets chained together with Mermaid's own invisible-link syntax, `~~~` -- e.g. `t1 ~~~ t6 ~~~ t9` for three anchors all at depth 2. This draws no visible edge and adds no relation of its own; it's a pure layout hint. A depth with only one anchor gets no chain (nothing to link), and an anchor whose depth can't be resolved -- see `arsgrammatica.compute_subordination_depths()`'s own docstring for when that happens, typically a relation cycle -- is left out of every chain and instead adds an extra entry to `warnings`, the same "degrade visibly, don't raise" convention `color_by_verbal_unit`'s over-8-verbal-units warning already uses above.
