"""
Tests for pipeline.py's analyze_string() -- the convenience wrapper that
...
analyze_string() is now a thin single-string convenience wrapper around
...
Two things worth checking that nothing else in the suite exercises:
  ...
  - the multi-sentence case analyzes every sentence it finds, in order,
    with each sentence's own SentenceAnalysis result lining up positionally.
"""
...
def test_analyze_string_single_sentence_returns_one_sentence_and_result():
    dspy.configure(lm=DummyLM([_ONE_SENTENCE, _ONE_SENTENCE_ANALYSIS]))

    sentences, results = analyze_string("arma virumque cano.", citation="ex.1")
...
def test_analyze_string_multi_sentence_analyzes_every_sentence_in_order():
    dspy.configure(lm=DummyLM([_TWO_SENTENCES, _ITA_ANALYSIS, _VALE_ANALYSIS]))

    sentences, results = analyze_string("ita. vale.", citation="ex.2")
